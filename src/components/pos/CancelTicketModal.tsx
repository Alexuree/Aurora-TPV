// =====================================================================
// Anulación segura de un ticket: muestra el ticket y lo marca como anulado.
// No borra el ticket.
// =====================================================================
...
export function CancelTicketModal({ sale, onClose, onCancelled }: { sale: Sale; onClose: () => void; onCancelled: () => void }) {
  ...
  const doCancel = async () => {
    setError(null);
    try {
      await cancel.mutateAsync({ saleId: sale.id, userId: user.id, userName: user.fullName });
      onCancelled();
      onClose();
    } catch (e) {
      ...
      footer={
        <>
          ...
          <Button variant="danger" disabled={cancel.isPending} onClick={doCancel}>
            {cancel.isPending ? 'Anulando…' : 'Anular ticket'}
          </Button>
        </>
      }
    >
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm">
          ...
          <div className="flex justify-between"><span className="text-slate-500">Pago</span><span className="font-medium">{sale.payments.map((p) => PAYMENT_LABELS[p.method] ?? 'Tarjeta').join(' + ')}</span></div>
